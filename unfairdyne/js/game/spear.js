...
function Spear(props) {
  ...
  this.initial_position = props.initial_position || {
    x: 320,
    y: 320
  };
  this.direction = props.direction || {
    x: 0,
    y: -1
  }; // normalized vector representing
...
Spear.prototype.collidesWithHeart = function() {
  ...
  */
  var displacement = {
    x: heart.pos_x - this.pos_x,
    y: heart.pos_y - this.pos_y
  };
  var normal_dir = {
    x: this.direction.y,
    y: -this.direction.x
  ...
  */
  var tip = {
    x: this.pos_x + this.direction.x * 23,
    y: this.pos_y + this.direction.y * 23
  };
...
function addNewSpear() {
  ...
  var position = {
    x: 320 + 160 * Math.cos(spawn_angle),
    y: 280 + 160 * Math.sin(spawn_angle)
  };

  var displacement = {
    x: heart.pos_x - position.x,
    y: heart.pos_y - position.y
  };
